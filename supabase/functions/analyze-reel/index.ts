...
function validateAndExtract(url: string): { valid: boolean; shortcode: string; error?: string } {
  try {
...
function generateIntroCard(score: number, rand: () => number): AnalysisCard {
  const descriptions: Record<string, string[]> = {
    ...
    mid: [
      ...
      "The intro is competent but lacks energy. The framing is fine, but there's no direct address or gesture to build viewer rapport.",
    ...
    low: [
      ...
      "The intro feels like a cold open with no setup. Viewers don't know why they should care within the first 3 seconds.",
  ...
  const recommendations: Record<string, string[]> = {
    ...
    low: [
      ...
      "Start with a gesture — a wave, a point, a prop. Movement in the first frame signals energy and triggers the algorithm's engagement boost.",
...
function generateRetentionCard(score: number, rand: () => number): AnalysisCard {
  ...
  const descriptions: Record<string, string[]> = {
    high: [
      ...
      `Predicted ${score}% watch-through. The content delivers on its hook promise, which is why viewers stay past the critical 5-second mark.`,
    ],
    mid: [
      `Predicted ${score}% watch-through rate. Minor drop-off detected at ~${dropoff}s.`,
      `Predicted ${score}% completion. The reel holds attention initially but loses ~${100 - score}% of viewers between ${dropoff}s and the end.`,
      `Predicted ${score}% watch-through. Pacing is steady but lacks the re-hook needed to retain viewers past the midpoint.`,
    ],
    low: [
      `Predicted ${score}% watch-through rate. Significant drop-off risk at ~${dropoff}s.`,
      `Predicted ${score}% completion. The retention curve drops sharply after ${dropoff}s — viewers are leaving before the content payoff.`,
      `Predicted ${score}% watch-through. Long static segments are causing viewers to scroll away. The reel needs more frequent visual changes.`,
    ],
  };
  const tagSets: Record<string, string[]> = {
  ...
  const recommendations: Record<string, string[]> = {
    high: [
      ...
      "Great watch-through. The algorithm will favor this content — consider boosting with a small ad spend to amplify organic reach.",
    ],
    mid: [
    ...
    low: [
      ...
      `Cut the reel shorter. If viewers leave at ${dropoff}s, make the reel ${dropoff + 2}s long. Shorter reels with high completion outperform longer ones with low completion.`,
      `Add a mid-reel hook — a question, a tease, or a visual surprise at ${dropoff}s. Without a re-hook, the algorithm sees the drop-off and limits reach.`,
    ],
  };
  const tier = score >= 75 ? "high" : score >= 50 ? "mid" : "low";
  const descArr = descriptions[tier];
...
function generateCompetitorCard(score: number, rand: () => number): AnalysisCard {
  ...
  const descriptions: Record<string, string[]> = {
    high: [
      ...
      `Top ${rank}% in your niche. Your trend alignment and audio choice match what's currently working for leading creators.`,
      `Above niche average. Your content is aligned with current trend signals and audio patterns that the algorithm is actively boosting.`,
    ],
    mid: [
      `Outperforms ${score}% of similar content in your niche. Room to close the gap with top performers.`,
      `Mid-tier in your niche. You're using some trending elements but missing the audio or format that top creators are adopting right now.`,
      `Average competitive position. Your content is relevant but not differentiated enough to stand out against top performers in your category.`,
    ],
    low: [
      `Below average for your niche. Top competitors are using strategies you haven't adopted yet.`,
      `Your trend alignment is behind the niche. Competitors are using trending audio and formats that you're not currently leveraging.`,
      `Bottom quartile in your niche. The content format and audio choice don't match what the algorithm is currently favoring for your category.`,
    ],
  };
  const tagSets: Record<string, string[]> = {
  ...
  const recommendations: Record<string, string[]> = {
    high: [
      ...
      "Keep your current strategy but experiment with one new format per week. Leaders who stop innovating get overtaken quickly.",
    ],
    mid: [
      ...
      "Adopt the trending format in your niche but add a unique twist. Direct copies get less algorithmic favor than creative adaptations.",
    ],
    low: [
      ...
      "Adopt the dominant reel format in your niche. Your current format is underrepresented in the top-performing content for your category.",
    ],
  };
...
function generateEnhancedReport(
  ...
  const postingTimeSuggestions: PostingTimeSlot[] = pickN(days, 3, rand).map((day) => {
  ...
  // Improvement suggestions — multiple variants per key for report uniqueness
  ...
  // Viral probability
  const viralProbability = Math.min(99, Math.round(
    overall * 0.35 +
    scores.hook * 0.2 +
  ...
  // Confidence metric — multiple reasoning variants per level
  ...
  // Final verdict — multiple variants per tier for report uniqueness
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const reelUrl = body.reelUrl;

    if (!reelUrl || typeof reelUrl !== "string") {
      return new Response(JSON.stringify({ error: "reelUrl is required" }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
    ...
    // Generate 5 sub-scores (reproducible for same shortcode, varied across different shortcodes)
    ...
    const dashboardMetrics = generateDashboardMetrics(
      { hook: hookStrength, caption: captionPower, intro: Math.round((hookStrength + visualQuality) / 2), retention: analysisCards[3].score, competitor: trendAlignment },
      rand
    );
    ...
    const dbRes = await fetch(`${supabaseUrl}/rest/v1/analyses`, {
      method: "POST",
      headers: {
    ...
    const historyRes = await fetch(`${supabaseUrl}/rest/v1/analyses?select=reel_url,overall_score,shortcode,created_at&order=created_at.desc&limit=5`, {
      headers: {
        apikey: supabaseKey,
        Authorization: `Bearer ${supabaseKey}`,
      },
    });
    const recentAnalyses = historyRes.ok ? await historyRes.json() : [];
